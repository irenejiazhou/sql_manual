...
-- 将原表的颗粒度从customer_id+visited_on变成visited_on
WITH t1 AS (
...
, t2 AS (
	SELECT visited_on,
		...
		sum(amount) OVER(ORDER BY visited_on ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS amount, -- 计算当前行和前六行
...
with t as (
	...
	sum(amount) over (order by visited_on RANGE between INTERVAL 6 DAY preceding and current row) as amount,-- 计算当前日期和前六天的所有行
...
-- METHOD 1和2的区别原因：一个用的是ROWS一个用的是RANGE
/*
