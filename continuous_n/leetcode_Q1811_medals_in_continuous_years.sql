...
with t as ( -- cols -> rows
	select contest_id, gold_medal as user_id, 'gold' as medal
	from contests
	
	union
	
	select contest_id, silver_medal as user_id, 'silver' as medal
	from contests
	
	union
	
	select contest_id, bronze_medal as user_id, 'bronze' as medal
	from contests
	order by user_id, contest_id)
, t1 as (
	select user_id, contest_id, row_number() over(partition by user_id order by contest_id) as rk
	from t)
, t2 as (
	-- The user won any medal in three or more consecutive contests.
	select user_id
	from t1
	group by user_id, contest_id - rk -- the continuous group tag
	having count(*) >= 3
